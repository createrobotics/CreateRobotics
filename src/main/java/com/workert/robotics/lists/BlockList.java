...
import com.workert.robotics.block.CodeEditor;
import com.workert.robotics.block.SmasherBlock;
import com.workert.robotics.block.computer.ComputerBlock;
import com.workert.robotics.block.inputblock.redstonedetector.RedstoneDetectorBlock;
import com.workert.robotics.block.inputblock.scanner.ScannerBlock;
...
public class BlockList {
	...
	public static final BlockEntry<DropExperienceBlock> TIN_ORE = Robotics.REGISTRATE.block("tin_ore",
			...
			.transform(tagBlockAndItem("ores/tin", "ores_in_ground/stone")).tag(Tags.Items.ORES).build().register();
	...
	public static final BlockEntry<SmasherBlock> SMASHER = Robotics.REGISTRATE.block("smasher", SmasherBlock::new)
			.lang("Smasher").blockstate((dataGenContext, provider) -> provider.horizontalBlock(dataGenContext.get(),
					provider.models().getExistingFile(provider.modLoc("block/smasher")))).properties(
					properties -> properties.of(Material.METAL).sound(SoundType.METAL).strength(9f)
							.requiresCorrectToolForDrops()).simpleItem().register();

	public static final BlockEntry<CodeEditor> CODE_EDITOR = Robotics.REGISTRATE.block("code_editor", CodeEditor::new)
			.lang("Code Editor").blockstate((dataGenContext, provider) -> provider.horizontalBlock(dataGenContext.get(),

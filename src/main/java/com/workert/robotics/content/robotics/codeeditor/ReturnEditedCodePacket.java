...
public class ReturnEditedCodePacket extends SimplePacketBase {
	...
	public void handle(Supplier<NetworkEvent.Context> context) {
		context.get().enqueueWork(() -> {
			if (context.get().getSender().getItemInHand(InteractionHand.MAIN_HAND).is(ItemList.PROGRAM.get()))
				context.get().getSender().getItemInHand(InteractionHand.MAIN_HAND).getOrCreateTag()
						.putString("code", this.code);
			else if (context.get().getSender().getItemInHand(InteractionHand.OFF_HAND).is(ItemList.PROGRAM.get()))
